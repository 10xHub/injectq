...
def test_inject_explicit_marker():
    ...
    container = InjectQ()
    container.bind_instance(MockService, MockService("explicit_value"))
    
    def explicit_function(service=Inject(MockService)) -> str:
        return service.get_value()
    
    # Note: This test shows the marker usage, but actual implementation
    # of explicit injection would require additional handling
    assert explicit_function.__defaults__[0].service_type == MockService
