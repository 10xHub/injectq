...
import pytest
from injectq import InjectQ, Module, SimpleModule, ProviderModule, ConfigurationModule, provider, inject, singleton
...
def test_module_composition():
    ...
    class DatabaseModule(Module):
        def configure(self, binder):
            binder.bind_instance("db_url", "postgresql://composed/db")
            binder.bind(MockDatabase, MockDatabase)
    
    # Service module  
    class ServiceModule(Module):
        def configure(self, binder):
            binder.bind_instance("timeout", 45)
...
def test_module_with_injection():
    ...
    class ProcessingModule(Module):
        def configure(self, binder):
            binder.bind_instance(str, "module://test")
            binder.bind_instance(int, 100)
            binder.bind(MockDatabase, MockDatabase)
...
def test_module_override_in_testing():
    ...
    test_module = (TestModule()
                   .bind_value(str, "sqlite://test.db")
                   .bind_value("test_mode", True))
